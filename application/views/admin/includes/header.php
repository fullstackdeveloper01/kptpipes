...
<div id="header">
   ...
   <ul class="nav navbar-nav navbar-right">
      ...
      $notidata = $this->db->get_where(db_prefix().'notifications',['isread'=>0])->result_array();
    ...
    ?>
      <li class="nav-item dropdown" style="position:relative;">
        ...
        <label class="badge badeg-primary" style="position:absolute;"><?=(count($notidata)>0)?count($notidata):''?></label>
        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
         <?php
            foreach ($notidata as $key => $value) {
               $result = databasetable($value);
               $name = $this->db->select($result['name'].' as name')->get_where($result['table'],['id'=>$value['touserid']])->row();
               ?>
               <a class="dropdown-item <?= ($value['isread_inline'] == 0) ? 'active' : 'inactive';?>" href="<?=site_url($value['link']).'/'.$value['id']?>">Greetings to <?=$name->name?></a>
               
         <?php }
         ?>
         <div class="bg bg-primary" style="padding: 5px;text-align: center;">
            <a href="<?=site_url('greeting-list')?>" style="color:#fff;" >View all</a>
         </div>
         </div>
      </li>
      <li> </li>
