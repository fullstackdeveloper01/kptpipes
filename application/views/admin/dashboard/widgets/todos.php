   ...
   <div class="panel_s todo-panel">
      <div class="panel-body padding-10">
        <h4 class="pull-left padding-5">
            <?php echo _l('Recently Joined User'); ?>
        </h4><span class="pull-right"><a href="<?= admin_url('clients');?>">View All</a></span>
        <div class="clearfix"></div>
        <hr class="hr-panel-heading-dashboard">
        <ul class="list-unstyled todo unfinished-todos todos-sortable sortable">
            <?php
                $userlists = $this->db->limit(5)->order_by('userid','desc')->get_where(db_prefix().'clients', array('active' => 1))->result();
                foreach($userlists as $rrr)
                {
                    ?>
                        <li class="media event">
                            <div class="media-body">
                                <!--<a class="title" href="<?= admin_url('clients/client/'.$rrr->userid); ?>">-->
                                <a class="title" href="javascript:void(0);">
                                    <strong><?= $rrr->company; ?></strong>
                                    <span class="pull-right">More info <i class="fa fa-arrow-circle-right"></i></span>
                                </a>
                                <p><?= $this->db->get_where(db_prefix().'contacts', array('userid' => $rrr->userid))->row('email'); ?></p>
                            </div>
                        </li>
                    <?php
                }
