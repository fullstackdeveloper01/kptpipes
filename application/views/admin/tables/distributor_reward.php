...
$join = [
    ...
    'LEFT JOIN '.db_prefix().'distributors ON '.db_prefix().'distributors.id='.db_prefix().'user_reward.user_id',
];

array_push($where, 'AND ('.db_prefix().'user_reward.status = "1" )');
array_push($where, 'AND ('.db_prefix().'user_reward.type = "distributor" )');
...
$result  = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where,[
    ...
    db_prefix().'user_reward.barcode_id',
    db_prefix().'user_reward.message',
    db_prefix().'distributors.distributor_name as name ',
]);
