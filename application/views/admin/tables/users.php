...
foreach ($rResult as $key => $aRow ) {
    ...
    if (empty($aRow['rolename'])) {
        $row[] = 'Super Admin';
    }else{
        $row[] = $aRow['rolename'];
    }
    $toggleActive = '<div class="onoffswitch" data-toggle="tooltip" data-title="' . _l('Change status') . '">
    <input type="checkbox" data-switch-url="' . admin_url() . 'user/change_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['staffid'] . '" data-id="' . $aRow['staffid'] . '" ' . ($aRow['active'] == 1 ? 'checked' : '') . '>
    ...
    $row[] = $toggleActive;
    $options = icon_btn('user/add/' . base64_encode($aRow['staffid']), 'pencil-square-o');
    $row[]   = $options .= icon_btn('user/delete_user/' . base64_encode($aRow['staffid']), 'remove', 'btn-danger _delete');
