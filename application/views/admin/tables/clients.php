...
$result = data_tables_init($aColumns, $sIndexColumn, $sTable, $join, $where, [
    ...
    db_prefix().'contacts.state as state_id',
    db_prefix().'contacts.city as city_id',
    'lastname',
...
*/
   ...
   // $row[] = ($aRow['contact_id'] ? '<a href="' . admin_url('clients/client/' . $aRow['userid'] . '?contactid=' . $aRow['contact_id']) . '" target="_blank">' . $aRow['firstname'] . ' ' . $aRow['lastname'] . '</a>' : '');
    ...
    <input type="checkbox"' . ($aRow['registration_confirmed'] == 0 ? ' disabled' : '') . 'onclick="userCounter()" data-switch-url="' . admin_url() . 'clients/change_client_status" name="onoffswitch" class="onoffswitch-checkbox" id="' . $aRow['userid'] . '" data-id="' . $aRow['userid'] . '" ' . ($aRow[db_prefix().'clients.active'] == 1 ? 'checked' : '') . '>
    ...
    //$row[] = statename($aRow['state_id']);
     
    //$row[] = cityname($aRow['city_id']);
    
    $row[] = $toggleActive;
   
    //$row[] = '<a href="' . admin_url('clients/client/' . $aRow['userid'] . ($isPerson && $aRow['contact_id'] ? '?group=contacts' : '')) . '">' . _l('Edit') . '</a> | <a href="' . admin_url('clients/delete/' . $aRow['userid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
    $row[] = '<a href="' . admin_url('clients/delete/' . $aRow['userid']) . '" class="text-danger _delete">' . _l('delete') . '</a>';
