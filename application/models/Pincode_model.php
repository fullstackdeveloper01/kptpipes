...
class Pincode_model extends App_Model
{
    ...
    {
        $this->db->select('*');
        $this->db->from(db_prefix() . 'area_pincode');
        if($id)
        {
            $this->db->where('id', $id);
        }
    ...
    {
        $this->db->insert(db_prefix() . 'area_pincode', $data);
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'area_pincode', $data);
