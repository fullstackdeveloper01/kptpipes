...
class Tax_model extends App_Model
{
    ...
    {
        $this->db->insert(db_prefix() . 'tax', $data);
        $insert_id = $this->db->insert_id();
        if ($insert_id) {
            log_activity('New tax Added [id: ' . $insert_id . ']');
        }
        return $insert_id;
    }
    ...
    {
        $this->db->where('id', $id);
        $this->db->update(db_prefix() . 'tax', $data);
        if ($this->db->affected_rows() > 0) {
            log_activity('tax Updated [id: ' . $id . ']');
    ...
    /**
    *   @Function: Update tax, image remove
    */
