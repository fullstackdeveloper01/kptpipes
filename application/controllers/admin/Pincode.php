...
class Pincode extends AdminController
{
    ...
    public function index()
    {
        ...
        if ($this->input->is_ajax_request()) {
            $this->app->get_table_data('pincode');
        }
      
        $subheader_text = setupTitle_text('aside_menu_active', 'master', 'pincode');
        ...
        $this->load->view('admin/pincode/pincodes', $data);
    }
    ...
    */
    ...
    public function getArealist()
    {
        ...
        $profileResult = $this->db->get_where(db_prefix().'area_new', array('city_id' => $city_id))->result();
        echo json_encode($profileResult);
    }
    ...
    public function add($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $id = $this->pincode_model->add_area($data);
                if ($id) {
                    set_alert('success', _l('added_successfully', _l('Pincode')));
                    redirect(admin_url('pincode'));
                }
            } else {
                ...
                $success = $this->pincode_model->update_area($data, $id);
                if ($success) {
                    set_alert('success', _l('updated_successfully', _l('Pincode')));
                }
                redirect(admin_url('pincode'));
            }
        }
        ...
        $data['article'] = $this->db->get_where(db_prefix().'area_pincode',array('id' => $id))->row();
    ...
    public function delete_area($id)
    {
        ...
        $response = $this->pincode_model->delete_area($id);
        if ($response == true) {
            set_alert('success', _l('deleted', _l('Pincode')));
        } else {
            set_alert('warning', _l('problem_deleting', _l('Pincode')));
        }
        redirect(admin_url('pincode'));
    }
}
