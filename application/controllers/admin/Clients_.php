...
class Clients extends AdminController
{
    ...
    public function index()
    {
        ...
        /*
        $custommenu = $this->db->get_where('tbloptions', array('name' => 'aside_menu_active'))->row('value');
        $mainmenu_ = json_decode($custommenu);
        $heading_text = $mainmenu_->customers->heading;
        if($heading_text != '')
        {
        ...
        {
            $data['heading_text'] = 'Customers';
        }
        */
        //$this->load->helper(array('admin_helper'));
        $header_text = title_text('aside_menu_active', 'Users');
    ...
    {
        $totaluser = $this->db->get_where(db_prefix().'clients')->num_rows();
        $totalactiveuser = $this->db->get_where(db_prefix().'clients', array('active' => 1))->num_rows();
        $totalinactiveuser = $this->db->get_where(db_prefix().'clients', array('active' => 0))->num_rows();
        echo json_encode([
            'success' => true,
            'message' => '',
            'totaluser' => $totaluser,
