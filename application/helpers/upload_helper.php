...
//
function handle_file_upload($taskid, $foldername, $index_name = 'attachments')
{
    $uploaded_files = [];
    //$path           = get_upload_path_by_type('task') . $taskid . '/';
    // echo 'uploads/'.$foldername.'/' . $taskid . '/'; 
    $path           = 'uploads/'.$foldername.'/' . $taskid . '/';
    $CI             = &get_instance();
    ...
    if (isset($_FILES[$index_name]['name'])
        ...
        for ($i = 0; $i < count($_FILES[$index_name]['name']); $i++) {
            ...
            if (!empty($tmpFilePath) && $tmpFilePath != '') {
                ...
                $filename    = unique_filename($path, str_replace(' ', '_', $_FILES[$index_name]['name'][$i]));
 ...
 * @return mixed
 */
function handle_recording_array($taskid, $index_name = 'attachments')
{
    $uploaded_files = [];
    //$path           = get_upload_path_by_type('task') . $taskid . '/';
    $path           = 'uploads/recording/' . $taskid . '/';
    $CI             = &get_instance();
