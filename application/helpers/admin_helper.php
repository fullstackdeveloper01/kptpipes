...
function get_admin_body_class($class = '')
{
	...
	function notify_type($id="")
    {
		...
		else if($id=='commentlike'){
		      $type= 'commentlike';
		}
		else if($id=='commentReply'){
		      $type= 'commentReply';
		}
		else if($id=='comment'){
		      $type= 'comment';
	 ...
	 */
    function sendNotification($userId, $message, $notify_type,$url,$time,$status,$id)
    {
    	$CI =& get_instance();
    	
        $device_id = $CI->db->get_where(db_prefix().'_patient', array('id' => $userId))->row('device_id');
...
*/
function title_text($type, $controllers_name)
{
    $CI = &get_instance();
    $custommenu = $CI->db->get_where('tbloptions', array('name' => $type))->row('value');
    $mainmenu_ = json_decode($custommenu);
  
    $heading_text_ = $mainmenu_->$controllers_name->heading;
    if($heading_text_ == '')
    {
        $heading_text_ = $controllers_name;
    }
    return  $heading_text_;
}

/**
*   Custom setup menu response
*/
function setupTitle_text($type, $controllers_name, $param)
{
    $CI = &get_instance();
    $custommenu = $CI->db->get_where('tbloptions', array('name' => $type))->row('value');
    $submainmenu_ = json_decode($custommenu);
  
    $subheading_text_ = $submainmenu_->$controllers_name->children->$param->heading;
    if($subheading_text_ == '')
    {
        $subheading_text_ = $controllers_name;
    }
    return  $subheading_text_;
}
...
function SelectOrder($orderid)
{
    ...
    return $CI->Orders_model->getDetails($orderid);
}
/* Product name */
function productNames($catid)
{
    $CI = &get_instance();
    $catname = $CI->db->get_where(db_prefix().'products', array('id' => $catid))->row('title');
    if($catname == '')
    {
        $catname = '-';
    }
...
/* Product Variant */
function productVariant($catid)
{
    $CI = &get_instance();
    $catname = $CI->db->get_where(db_prefix().'products', array('id' => $catid))->row('product_variant');
    if($catname == '')
    {
        $catname = '-';
    }
...
function userReward($id){
    $CI = &get_instance();

    // this code is for get order details
    $orderData = $CI->db->get_where(db_prefix().'orders_history',['id'=>$id])->row_array();

    // this code is for get distributor reward point
    $discountPercent =$CI->db->select('percent,id')->get_where(db_prefix().'reward',['status'=>1,'isDeleted'=>0,'user_type'=>$orderData['type'],'product_id'=>$orderData['product_id']])->row();

    // this code is for get distributor current stock
    $totalQty =$CI->db->get_where(db_prefix().'stocks',['bach_no'=>$orderData['bach_no']])->row('quantity');

    $updateQty['quantity']= $totalQty-$orderData['quantity'];

    // this code is for update admin stock
    $res = $CI->db->where(['bach_no'=>$orderData['bach_no']])->Update(db_prefix().'stocks',$updateQty);
...
/*this is for Check Email id */
function checkEmail($email){
    $CI = &get_instance();
    $where['dealer_email'] = $email;
    $where['status'] = 1;
    $count =  $CI->db->get_where(db_prefix().'dealer', $where)->num_rows();
    
    $where1['distributor_email'] = $email;
    $where1['status'] = 1;
    $count1 =  $CI->db->get_where(db_prefix().'distributors', $where1)->num_rows();

    $where2['plumber_email'] = $email;
    $where2['status'] = 1;
    $count2 = $CI->db->get_where(db_prefix().'plumber', $where2)->num_rows();

    $where3['email'] = $email;
    $where3['active'] = 1;
    $count3 = $CI->db->get_where(db_prefix().'staff', $where3)->num_rows();
...
/*this is for Check mobile Number */
function checkMobile($mobile,$id=null){
    $CI = &get_instance();
    $where['distributor_mobile'] = $mobile;
    $where['status'] = 1;
    $count = $CI->db->get_where(db_prefix().'distributors', $where)->num_rows();

    $where1['dealer_mobile'] = $mobile;
    $where1['status'] = 1;
    $count1 = $CI->db->get_where(db_prefix().'dealer', $where1)->num_rows();
...
/*this is for check Aadhar Number */
function checkAadharNumber($aadhar){
    $CI = &get_instance();
...
/*this is for check pan Number */
function checkPanNumber($pan){
    $CI = &get_instance();
...
function checkPermissions($request){
    $CI = &get_instance();
    $staffid = $CI->session->staff_user_id;
    if ($CI->session->staff_role_id != 0) {
        $permissions = $CI->db->get_where(db_prefix().'roles',['roleid'=>$CI->session->staff_role_id])->row('permissions');
